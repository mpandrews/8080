...
Screen::Screen(const unsigned char* SIVideoBuffer)
{
	...
	if(window == NULL)
	{
		std::cout << "Could not load SDL window. "
			  << SDL_GetError()
			  << std::endl;
		exit(1);
	}
	...
	if(renderer == NULL)
	{
		std::cout << "Could not load SDL renderer. "
			  << SDL_GetError()
			  << std::endl;
		exit(1);
	}
	...
	// the renderer.
	this->surface = SDL_CreateRGBSurfaceWithFormatFrom(
		this->displayBuffer,
		SI_SCREEN_WIDTH,
		SI_SCREEN_HEIGHT,
		0,
	...
	if(surface == NULL)
	{
		std::cout << "Could not load SDL surface. "
			  << SDL_GetError()
			  << std::endl;
		exit(1);
	}
		...
		for(int j = 0; j < 8; ++j)
		{
			...
			if(pixels & bitMasks[j])
			{
				...
				if(col < this->topScreenFilterEnd)
					colorPixelValue = this->topScreenColorFilter;
				else if(col > this->bottomScreenFilterBegin)
					colorPixelValue = this->bottomScreenColorFilter;
				else
