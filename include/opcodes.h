...
static inline uint8_t* fetch_operand(uint8_t operand_field,
		const struct cpu_state* cpu)
{
	switch(operand_field)
	{
		case OPERAND_REG_B:
			return &HIGH_REG8(cpu->bc);
		case OPERAND_REG_C:
			return &LOW_REG8(cpu->bc);
		case OPERAND_REG_D:
			return &HIGH_REG8(cpu->de);
		case OPERAND_REG_E:
			return &LOW_REG8(cpu->de);
		case OPERAND_REG_H:
			return &HIGH_REG8(cpu->hl);
		case OPERAND_REG_L:
			return &LOW_REG8(cpu->hl);
		case OPERAND_MEM:
			return cpu->memory + cpu->hl;
		case OPERAND_REG_A:
			return &HIGH_REG8(cpu->psw);
		default:
			fprintf(stderr, "ERROR: fetch_operand()" 
			"was passed %#2x, which is"
			"out of range!", operand_field);
			exit(1);
